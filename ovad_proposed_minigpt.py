...
def main():
    ...
    # configure file
    predict_file_name = f'results/{cfg.dataset_name}/minigpt_ovad_{cfg.dataset_name}_{cfg.prompt_type}.json'
    ...
    if cfg.anomaly_detect:
        # load lvlm
        model, vis_processor = load_lvlm(cfg)
            ...
            for i, video_path in progress_bar(enumerate(video_paths), total=len(video_paths)):
                ...
                for cp in cp_loader:
                    ...
                    for keyword in keyword_list:
                        ...
                        response_wa = lvlm_test(model, vis_processor, instruction, None, wa_image)
                        response_tc = lvlm_test(model, vis_processor, instruction_tc, None, grid_image)

                        score = generate_output(response[0])['score']
                        score_wa = generate_output(response_wa[0])['score']
                        score_tc = generate_output(response_tc[0])['score']
    ...
    if cfg.calc_auc:
        ...
        graph_path = f'results/{cfg.dataset_name}/{cfg.type}/{cfg.prompt_type}/minigpt_ovad_org_{cfg.dataset_name}_{cfg.type}_{cfg.prompt_type}.jpg'
        ...
        print('-----------------------------------')
        anomalies_idx = [i for i,l in enumerate(labels) if l==1] 
        graph_path = f'results/{cfg.dataset_name}/{cfg.type}/{cfg.prompt_type}/minigpt_ovad_combi_{cfg.dataset_name}_{cfg.type}_{cfg.prompt_type}.jpg'
    ...
    if cfg.calc_video_auc:
        ...
        for i in progress_bar(range(len(label_arr)), total=len(label_arr)):
            ...
            graph_path = f'results/{cfg.dataset_name}/{cfg.type}/{cfg.prompt_type}/videos/minigpt_ovad_combi_{cfg.dataset_name}_{video_name}_{cfg.type}_{cfg.prompt_type}.jpg'
